...
class QuickFormTest extends KernelTestBase {
  ...
  /**
   * Test quick form entity validation.
   */
  public function testQuickFormEntityValidation() {

    // Assert that an EntityMalformedException will be thrown.
    $this->expectException(EntityMalformedException::class);
    $this->expectExceptionMessage('The asset generated by quick form test_entity_validation failed validation.');

    // Programmatically submit the test_entity_validation quick form.
    ...
    // We ignore this because we are following Drupal core's pattern.
    // @phpstan-ignore arguments.count
    \Drupal::formBuilder()->submitForm('\Drupal\farm_quick\Form\QuickForm', $form_state, 'test_entity_validation');
  }
